...
namespace CK.Setup
{
    ...
    public class AutoServiceClassInfo : IStObjServiceFinalSimpleMapping
    {
        ...
        internal bool EnsureCtorBinding( IActivityMonitor m, CKTypeCollector collector )
        {
            ...
            if( ctors.Length > 1 ) m.Error( $"Multiple public constructors found for '{ClassType.FullName}'. Only one must exist. Consider using factory methods that relays to private constructors for explicit initialization." );
            else
            {
