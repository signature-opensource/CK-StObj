...
sealed class GeneratedRootContext : IStObjMap, IStObjObjectMap, IStObjServiceMap
{
    ...
    // Note: When no DIContainerDefinition exist, the code that manages endpoints is not generated.
    public bool ConfigureServices( in StObjContextRoot.ServiceRegister reg )
    {
        ...
        var mappings = EndpointHelper.CreateInitialMapping( reg.Monitor, reg.Services, DIContainerHub_CK._endpointServices.ContainsKey );
        ...
        //  - Then an instance of the special "super singleton" DIContainerHub is created.
        ...
        //    and the true instance singletons IStObjMap, DIContainerHub, the EndpointType and the IEnumerable<IDIContainer>. 
        foreach( IDIContainerInternal e in theEPTM._containers )
        {
